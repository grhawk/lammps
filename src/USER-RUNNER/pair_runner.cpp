...
//
// Copyright (c) 2013, 2014 Andreas Singraber
//
////////////////////////////////////////////////////////////////////////////////
...
void PairRuNNer::RuNNer_symfunc::setup(char *line) {
    ...
    if(type==2) {
        ...
        if(abs(zeta - int(zeta)) < SMALL_DOUBLE) {
            optpow = int(zeta);
        }
    }
...
void PairRuNNer::RuNNer_symfuncGroup::info() {
    ...
    if(type == 2) {
        fprintf(par.screen, "%4d %2s %2d %2s                  *       * %7.3f %4d\n", num, par.e[ec]->name, type, par.e[e1]->name, rc, numSF);
        for(int i=0; i<numSF; i++) {
            fprintf(par.screen, "                         %7.3f %7.3f              %4d %4d\n", eta[i], rs[i], i, index[i]);
        }
    }
    else if(type == 3) {
        fprintf(par.screen, "%4d %2s %2d %2s %2s       *       *       * %7.3f %4d\n", num, par.e[ec]->name, type, par.e[e1]->name, par.e[e2]->name, rc, numSF);
        for(int i=0; i<numSF; i++) {
            fprintf(par.screen, "                 %7.3f %7.3f %7.3f              %4d %4d %4d\n", eta[i], lambda[i], zeta[i], i, index[i], etaind[i]);
        }
    }
